...
async def signup(user: UserDTO) -> SignupResponseDTO:
    ...
    if user.email:
        ...
        raise HTTPException(status_code=404, detail="Email or mobile is required")
...
async def verify(otp: OtpDTO) -> VerifyResponseDTO:
    ...
    :raises HTTPException: If email or mobile is not provided or if the OTP is invalid.
    """
    if otp.email:
        key = otp.email
    elif otp.mobile:
    ...
    if key not in otp_dict or otp_dict[key] != otp.otp:
        raise HTTPException(status_code=400, detail=f"Invalid {key} or OTP.")
